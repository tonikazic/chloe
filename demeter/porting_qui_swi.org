# this is ../c/maize/demeter/porting_qui_swi.org
...
*** GNG Test pedigree construction by crop and rowplant, excluding family numbers



#+BEGIN_SRC prolog
test_pedigrees([1,2,3],'19r').
#+END_SRC

Added missing Knums to founders in [[file:data/genotype.pl][genotype.pl]] <2018-12-10 Mon>.  Checks out:
...
4 ?- findall(F,genotype(F,_,_,_,_,_,_,_,_,_,'K00'),Fs),write_list(Fs).
Fs = [].
...
Decided to write a set of predicates that can stand alone, and leave
calling after pedigrees:build_pedigrees/2 or during pedigrees:pretty_pedigree/4 for
later (<2018-12-10 Mon>).  My rationale is that checking will be mostly a
one-time thing....
