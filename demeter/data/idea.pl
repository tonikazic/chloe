...
idea(19,'TAG THE MUTANTS FIRST!  The tags need to go on as soon as possible for scoring to commence.  The inbreds can be tagged after the first ears appear, which will simplify getting the tags in the ear zone.   Early scoring is crucial to managing pollinations and data collection.  Consider marking scoring before tags go on and then collecting the data once tagged; or using labels as light-weight intermediate tags for mutants.',toni,date(10,8,2009),time(4,30,0)).
...
% idea(,'',date(19,4,2013),time(05,31,00)).
